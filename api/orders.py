...
logger = loguru.logger.bind(name="srv.cdek.orders")
...
class OrdersMixin:
	...
	async def get_order(cls, uuid: str) -> OrderInfo:
		try:
			response = await cls.get(f"/v2/orders/{uuid}")
	...
	async def get_order_by_cdek_number(cls, cdek_number: str) -> list[OrderInfo]:
		try:
			response = await cls.get("/v2/orders", params={"cdek_number": cdek_number})
	...
	async def get_orders(
		...
		try:
			...
			response = await cls.get("/v2/orders", params=search_params)
			if isinstance(response, list):
				return [OrderInfo(**order) for order in response]
			return [OrderInfo(**response)]
		except ValidationError as e:
	...
	async def delete_order(cls, uuid: str) -> OrderResponse:
		try:
			response = await cls.delete(f"/v2/orders/{uuid}")
			return OrderResponse(**response)
		except ValidationError as e:
